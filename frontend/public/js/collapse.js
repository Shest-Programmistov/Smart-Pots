function createCollapseButton(id) {
    ...
    let collapseButton = createElement('button', { 'className': 'btn btn-link collapsed' , 'innerHTML': arrowDown});

    collapseButton.addEventListener('click', () => {
        $('#collapse' + id).collapse('toggle');

        if (collapseButton.innerHTML == arrowDown)
            collapseButton.innerHTML = arrowUp;
        else 
            collapseButton.innerHTML = arrowDown;
    });
